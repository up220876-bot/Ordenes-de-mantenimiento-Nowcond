...
def crear_datos_iniciales():
    ...
    if ControlDocumental.query.count() == 0:
        db.session.add(ControlDocumental(
            codigo="RC 7.1.3.b",
            fecha_emision="17/03/2020",
            revision="5",
            fecha_revision="25/09/2025",
            elaboro="Ing. Eusebio Herrera",
            reviso="Ing. Carlos Santacruz"
        ))
...
def nueva_orden():
    ...
    return render_template(
        ...
        control=ControlDocumental.query.first()
    )
...
@app.route("/admin_control_footer", methods=["GET", "POST"])
def admin_control_footer():
    if session.get("rol") != "admin":
        return "Acceso no autorizado"
    ...
    if request.method == "POST":
        control.codigo = request.form.get("codigo")
        control.fecha_emision = request.form.get("fecha_emision")
        control.revision = request.form.get("revision")
        control.fecha_revision = request.form.get("fecha_revision")
        control.elaboro = request.form.get("elaboro")
        control.reviso = request.form.get("reviso")

        db.session.commit()

        return redirect(url_for("administrar_formatos"))

    return render_template("admin_control_footer.html", control=control)
